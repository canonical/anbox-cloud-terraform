...
locals {
  subcluster_config_map = { for obj in var.subclusters : obj.name => {
    ...
    registry_config = var.deploy_registry && obj.registry != null ? obj.registry.enable ? { mode = obj.registry.mode, offer_url = obj.registry.mode == "client" ? one(module.registry[*].client_offer_url) : one(module.registry[*].publisher_offer_url) } : null : null
  } }
}
...
module "subcluster" {
  ...
  registry_connection = each.value.registry_config
